...
gulp.task('production', ['images'], function(){
  gulp.src(['./' + destinationdirectory + '/js/app.js'])
    // Constant
    .pipe(replace('"BUILDTYPE", "development"', '"BUILDTYPE", "production"'))
    .pipe(replace('"APIROUTE", "http://localhost:9090/"', '"APIROUTE", "http://localhost/copark/api/"'))
    .pipe(replace('"BASEHREF", "/"' , '"BASEHREF", "/godesk/"'))
    .pipe(gulpif('*.js', uglify()))
    .pipe(gulp.dest('./' + destinationdirectory + '/js'));
    // Base
    gulp.src(['./' + destinationdirectory + '/index.html'])
    .pipe(replace('<base href="/">', '<base href="/godesk/">'))
    .pipe(gulp.dest('./' + destinationdirectory ));      
});
...
gulp.task('manifest', ['production'], function(){
  return gulp
    .src(['./' + destinationdirectory + '/**/*'], { base: './public' })
    .pipe(manifest({
      hash: true,
      preferOnline: true,
      network: ['*'],
